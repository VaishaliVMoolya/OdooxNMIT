...
Renders an interactive, dynamic Dayflow HRMS interface (Attendance + Time Off + HR Approval Workflow)
with real live check-in/out ticker, leave applications, HR comments, and role switching.
"""
...
<head>
    ...
    <title>Dayflow HRMS — Functional Live UI Preview</title>
    ...
    <style>
        ...
        * {
        ...
        .status-dot.yellow { background-color: var(--accent-yellow); }
...
<body>
    ...
    <div class="container">
        ...
        <div id="tab-attendance" class="tab-content active">
            <div class="page-header">
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <button class="btn btn-secondary" onclick="filterAttendanceView('daily')">Daily View</button>
                    <button class="btn btn-secondary" onclick="filterAttendanceView('weekly')">Weekly View</button>
                    <button class="btn btn-secondary" onclick="resetAttendanceData()">Reset Data</button>
                </div>
            </div>

            <!-- Attendance Interactive Banner -->
            ...
            <div class="card" id="apply-leave-card">
                ...
                <form onsubmit="handleLeaveSubmit(event)">
                    ...
                    <div class="checkbox-group" style="margin-bottom: 1.25rem;">
                        <input type="checkbox" id="leave-attachment">
                        <label for="leave-attachment" style="font-size:0.85rem; color: var(--text-secondary);">Medical Certificate / Supporting File Attached</label>
                    </div>
        ...
        <!-- PLACEHOLDER TABS FOR OTHER DOMAINS -->
        <div id="tab-employees" class="tab-content">
            <div class="card"><h2>Employees Module</h2><p style="color:var(--text-secondary); margin-top:0.5rem;">Managed by Person 2 (Employee / Profile & Account Provisioning).</p></div>
        </div>
        <div id="tab-payroll" class="tab-content">
            <div class="card"><h2>Payroll & Salary</h2><p style="color:var(--text-secondary); margin-top:0.5rem;">Managed by Person 4 (Admin Dashboard & Payroll).</p></div>
        </div>
        <div id="tab-documents" class="tab-content">
            <div class="card"><h2>Documents</h2><p style="color:var(--text-secondary); margin-top:0.5rem;">Managed by Person 2 (Employee Documents).</p></div>
        </div>
    ...
    <script>
        ...
        const DEFAULT_LEAVE = [
            { id: 101, employee: 'John Doe', type: 'sick', startDate: '2026-08-25', endDate: '2026-08-26', days: 2, remarks: 'Fever and rest recommended', attachment: true, status: 'pending', adminComments: '' },
            { id: 102, employee: 'Jane Smith', type: 'paid', startDate: '2026-08-28', endDate: '2026-08-30', days: 3, remarks: 'Family vacation', attachment: false, status: 'approved', adminComments: 'Approved by HR' }
        ];
        ...
        function resetAttendanceData() {
            ...
            localStorage.removeItem('df_leaves');
        ...
        function handleCheckIn() {
            ...
            // Add active row to attendance list
        ...
        function updateLiveTicker() {
            ...
            const timeStr = `${hrs}h ${mins.toString().padStart(2, '0')}m ${secs.toString().padStart(2, '0')}s`;
            document.getElementById('banner-worked-hours').innerText = timeStr;

            // Update worked hours on active record
        ...
        function handleCheckOut() {
            ...
            if (activeRec) {
                ...
                // For demo/testing: if checked out after checking in, display worked hours and set Present status
                ...
                if (activeRec.workedHours >= 4.0 || diffSec > 0) {
        ...
        function handleLeaveSubmit(e) {
            ...
            const attachment = document.getElementById('leave-attachment').checked;
        ...
        function handleApproveLeave(leaveId) {
            ...
            if (leave) {
                ...
                // Auto-generate Attendance entries with 'leave' status
                ...
                saveState();
                renderAll();
                alert('Leave request approved! Attendance records updated.');
            }
        }
        ...
        function renderAttendanceTable() {
            ...
            tbody.innerHTML = filtered.map(a => {
                ...
                const statusLabel = a.status.toUpperCase().replace('_', ' ');
                ...
                return `
                    <tr>
                        ...
                        <td style="color: ${a.extraHours > 0 ? 'var(--accent-green)' : 'inherit'}; font-weight: ${a.extraHours > 0 ? '700' : 'normal'};">
                            ${a.extraHours}h
                        </td>
                    </tr>
                `;
            ...
            tbody.innerHTML = filtered.map(l => {
                ...
                return `
                    <tr>
                        ...
                        <td>${l.days} day(s)</td>
                        <td>${l.remarks}</td>
                        <td>${l.attachment ? '📎 Certificate' : 'None'}</td>
